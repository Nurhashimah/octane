class IndenCardsController < ApplicationController
  ...
  caches_action :index, :cache_path => proc {|c|
      timestamp = IndenCard.maximum(:updated_at).to_i
      string = timestamp.to_s + c.params.inspect+"_#{IndenCard.count}"+chkeys([Unit, Staff])
  ...
  def index
    @search = IndenCard.search(params[:q])
    @inden_cards = @search.result
  end
