class IndenUsagesController < ApplicationController
  ...
  caches_action :index, :cache_path => proc {|c|
      timestamp = IndenUsage.maximum(:updated_at).to_i
      string = timestamp.to_s + c.params.inspect+"_#{IndenUsage.count}"+chkeys([UnitFuel, Unit, Staff])
  ...
  def index
    ...
    respond_to do |format|
      if is_admin=="1" || current_user.staff.unit_id
        ...
        format.html {redirect_to root_path, notice: (t 'inden_usages.title')+(t 'users.staff_required')}
      end
    end
  end
